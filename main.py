...
class CPU_Scheduler_GUI:
    def __init__(self, root):
        ...
        self.scheduler_type_label = tk.Label(self.scheduler_frame, text="Select Scheduler Type:", font=("Arial", 12))
        self.scheduler_type_label.grid(row=0, column=0, padx=10, pady=10)
        ...
        self.num_process_label = tk.Label(self.scheduler_frame, text="Enter Number of Processes:", font=("Arial", 12))
        self.num_process_label.grid(row=1, column=0, padx=10, pady=10)

        self.num_process_entry = tk.Entry(self.scheduler_frame, font=("Arial", 12), width=20)
        self.num_process_entry.grid(row=1, column=1, pady=5)

        # Next button to move to process entry page
        self.next_button = ttk.Button(self.scheduler_frame, text="Next", command=self.show_process_page, width=20)
        self.next_button.grid(row=2, columnspan=2, pady=20)

        # Frame for entering processes
        self.process_label = tk.Label(self.process_frame, text="Enter Processes Data", font=("Arial", 14))
    ...
    def show_process_page(self):
        ...
        # Hide the scheduler settings page
        ...
        for i in range(num_processes):
            ...
            arrival_time_label = tk.Label(process_frame, text=f"Arrival Time for Process {i+1}:", font=("Arial", 12))
            arrival_time_label.grid(row=0, column=0, padx=5)

            arrival_time_entry = tk.Entry(process_frame, font=("Arial", 12), width=10)
            arrival_time_entry.grid(row=0, column=1, padx=5)

            # Burst Time Field (always displayed)
            burst_time_label = tk.Label(process_frame, text=f"Burst Time for Process {i+1}:", font=("Arial", 12))
            burst_time_label.grid(row=1, column=0, padx=5)

            burst_time_entry = tk.Entry(process_frame, font=("Arial", 12), width=10)
            ...
            if self.scheduler_type_var.get() in ["Priority Preemptive", "Priority non_Preemptive"]:
                priority_label = tk.Label(process_frame, text=f"Priority for Process {i+1}:", font=("Arial", 12))
                priority_label.grid(row=2, column=0, padx=5)

                priority_entry = tk.Entry(process_frame, font=("Arial", 12), width=10)
                priority_entry.grid(row=2, column=1, padx=5)
                self.processes.append((arrival_time_entry, burst_time_entry, priority_entry))
            else:
            ...
            if self.scheduler_type_var.get() == "Round Robin":
                quantum_label = tk.Label(process_frame, text=f"Quantum for Process {i+1}:", font=("Arial", 12))
                quantum_label.grid(row=3, column=0, padx=5)

                quantum_entry = tk.Entry(process_frame, font=("Arial", 12), width=10)
                quantum_entry.grid(row=3, column=1, padx=5)
                self.processes[-1] += (quantum_entry,)

        # Add "Add Processes" button
        self.add_process_button = ttk.Button(self.process_frame, text="Add Processes", command=self.add_processes)
        self.add_process_button.pack(pady=10)

    def add_processes(self):
        # Collect process details and add them to the list
        process_data = []
        for i, (arrival_time_entry, burst_time_entry, priority_entry, *quantum_entry) in enumerate(self.processes):
            arrival_time = arrival_time_entry.get()
            burst_time = burst_time_entry.get()

            # Validate the inputs
            if not arrival_time or not burst_time:
                messagebox.showerror("Error", f"Please enter both arrival time and burst time for Process {i+1}.")
                return
            try:
                arrival_time = int(arrival_time)
                burst_time = int(burst_time)
            except ValueError:
                messagebox.showerror("Error", f"Invalid input: Arrival Time and Burst Time must be integers for Process {i+1}.")
                return
            ...
            if priority_entry:
                ...
                if not priority:
                    messagebox.showerror("Error", f"Please enter priority for Process {i+1}.")
                    return
                try:
                    priority = int(priority)
                except ValueError:
                    messagebox.showerror("Error", f"Invalid input: Priority must be an integer for Process {i+1}.")
                    return
            ...
            if self.scheduler_type_var.get() == "Round Robin":
                ...
                if not quantum:
                    messagebox.showerror("Error", f"Please enter quantum for Process {i+1}.")
                    return
                try:
                    quantum = int(quantum)
                except ValueError:
                    messagebox.showerror("Error", f"Invalid input: Quantum must be an integer for Process {i+1}.")
                    return
        ...
        for data in process_data:
            print(f"Process {data['Process']} - Arrival Time: {data['Arrival Time']}, "
